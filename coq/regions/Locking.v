  ...
  + rename l0 into Ls.
    ...
    - Case "T_Call".
      ...
      * rewrite <- locks_subst. rewrite <- locks_subst.
        eapply wfProgram_wfMethodDecl in H18...
        inv H18. apply locks_static in H12. rewrite H12.
        contra...
    - Case "T_Update".
      inv Hfree. destruct_var; try(congruence).
      apply wfLocks_econtext in wfL...
...
Proof with eauto using wfLocking_heapExtend.
  ...
  + rename l into Ls.
    ...
    - Case "T_Locked".
      inverts H1.
      assert (wfWl': wfLocks Ls e) by eauto using wfLocks_econtext.
      inv_step; try(malformed_context)...
      * inv_eq.
        assert(Hno_locks: no_locks e1 /\ no_locks e2)
          by eauto using spawn_no_locks.
        assert(locks (ELocked (l, r) e) = locks (ELocked (l, r) e3))
      ...
      * inv_eq.
        ...
        assert(HnIn': ~In (l, r) (locks e')).
          inv wfL. assert (In (l, r) Ls)...
