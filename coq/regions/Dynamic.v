...
(***********)
  | EvalLock :
      forall H V n x l r e c F RL Ls,
        V x = Some (VLoc l) ->
        heapLookup H l = Some (c, F, RL) ->
        RL r = Some LUnlocked ->
        ~ In (l, r) Ls ->
        P / (H, V, n, T_Thread Ls (ELock (DV x) r e)) ==>
          (heapUpdate H l (c, F, extend RL r LLocked), V, n, T_Thread ((l, r) :: Ls) (ELocked (l, r) e))
  | EvalLock_Reentrant :
      forall H V n x l r e c F RL Ls,
        V x = Some (VLoc l) ->
        heapLookup H l = Some (c, F, RL) ->
        ...
        P / (H, V, n, T_Thread Ls (ELock (DV x) r e)) ==>
          (H, V, n, T_Thread Ls e)
  | EvalLock_Release :
      forall H V n l r v c F RL Ls,
        heapLookup H l = Some (c, F, RL) ->
        RL r = Some (LLocked) ->
        In (l, r) Ls ->
        P / (H, V, n, T_Thread Ls (ELocked (l, r) (EVal v))) ==>
          (heapUpdate H l (c, F, extend RL r LUnlocked), V, n, T_Thread (remove id_eq_dec (l, r) Ls) (EVal v))
(**************)
(* Exceptions *)
(**************)
  | EvalEXN_AsyncLeft :
...
Inductive cfg_blocked : configuration -> Prop :=
  ...
  | Blocked_Right :
      forall H V n T1 T2 e,
        ...
        cfg_blocked (H, V, n, T_Async T1 T2 e)
  | Blocked_W :
      forall H V n x e l r c F RL Ls,
        V x = Some (VLoc l) ->
        heapLookup H l = Some (c, F, RL) ->
        RL r = Some (LLocked) ->
        ~ In (l, r) Ls ->
        cfg_blocked (H, V, n, T_Thread Ls (ELock (DV x) r e))
  | Blocked_Context :
