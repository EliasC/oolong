...
Lemma wfProgram_wfMethodDecl :
  forall P t' c n ms m mtd,
    wfProgram P t' ->
    methods P (TClass c n) = Some ms ->
...
Corollary dyn_wfFieldLookup :
  forall P Gamma c F fs f t,
    ...
    fields P (TClass c false) = Some fs ->
...
Lemma methodSigs_n :
  forall P i n1 n2 msigs1 msigs2,
    methodSigs P (TInterface i n1) msigs1 ->
    methodSigs P (TInterface i n2) msigs2 ->
    msigs1 = msigs2.
Proof with eauto.
  introv Hsigs1 Hsigs2.
  gen msigs2. gen n2.
  inductions Hsigs1; intros; inv Hsigs2; try rewrite_and_invert.
  rewrite IHHsigs1_1 with (i := i3) (n1 := n0) (n2 := n4) (msigs2 := msigs3)...
  rewrite IHHsigs1_2 with (i := i4) (n1 := n2) (n2 := n5) (msigs3 := msigs4)...
Qed.

Lemma methodSigs_nullable :
  forall P t msigs,
    methodSigs P (nullable t) msigs ->
    methodSigs P t msigs.
Proof with eauto.
  introv Hsigs. unfolds nullable.
  inductions Hsigs; destruct t; try congruence; try inv_eq...
Qed.
...
Proof with eauto.
  ...
  apply methodSigs_n with (msigs1 := msigs1) (n1 := n1) in H3...
  apply methodSigs_n with (msigs2 := msigs2) (n2 := n2) in H5...
  subst...
  apply methodSigs_n with (msigs1 := msigs1) (n1 := n1) in H3...
  apply methodSigs_n with (msigs2 := msigs2) (n2 := n2) in H5...
  subst...
...
Proof with eauto.
  ...
  split.
  + intros wfT.
    inv wfT as [c n cLookup|i n iLookup|]...
...
Proof with eauto using
  ...
  + Case "Sub_Nullable".
    apply IHHsub with (msigs2 := msigs2)...
  + Case "Sub_Nullable_R".
    apply IHHsub with (msigs2 := msigs2)...
Qed.
...
Proof with eauto using
  ...
  assert (fields P (TClass c false) = Some fs)
...
Lemma wfFields_envExtend :
  forall P t' Gamma c n F l c',
    wfProgram P t' ->
    wfFields P Gamma c F ->
    wfEnv P Gamma ->
    fresh Gamma (env_loc l) ->
    wfType P (TClass c' n) ->
    wfFields P (extend Gamma (env_loc l) (TClass c' n)) c F.
...
Proof with eauto.
  ...
  destruct t...
  symmetry in Heqt.
  assert (tClass: exists c n, t = TClass c n) by (inv wfGamma; eauto).
  inv tClass as [c'' [n'']].
...
Lemma wfHeap_extend :
  forall P t' Gamma H c F L,
    wfProgram P t' ->
    wfHeap P Gamma H ->
    wfType P (TClass c false) ->
    wfFields P Gamma c F ->
    wfHeap P (extend Gamma (env_loc (length H)) (TClass c false)) (heapExtend H (c, F, L)).
Proof with eauto with env.
...
Lemma wfVars_heapExtend :
  forall Gamma P t' n V l c nll,
    wfProgram P t' ->
    wfVars P Gamma n V ->
    fresh Gamma (env_loc l) ->
    wfType P (TClass c nll) ->
    wfVars P (extend Gamma (env_loc l) (TClass c nll)) n V.
Proof with eauto with env.
...
Lemma wfThreads_heapExtend :
  forall P t' Gamma T t c n l,
    wfProgram P t' ->
    wfType P (TClass c n) ->
    fresh Gamma (env_loc l) ->
    wfThreads P Gamma T t ->
    wfThreads P (extend Gamma (env_loc l) (TClass c n)) T t.
...
Lemma wfConfiguration_heapExtend :
  forall P t' Gamma H V n T t c F L,
    ...
    wfType P (TClass c false) ->
    wfFields P Gamma c F ->
    wfConfiguration P (extend Gamma (env_loc (length H)) (TClass c false))
