...
Inductive wfEnv (P : program) : env -> Prop :=
  | WF_Env :
      (forall Gamma,
        ...
        (forall l t, Gamma (env_loc l) = Some t -> exists c n, t = TClass c n) ->
        wfEnv P Gamma).
...
Inductive subtypeOf (P : program) : ty -> ty -> Prop :=
  | Sub_Class :
      forall c i fs ms,
        ...
        subtypeOf P (TClass c false) (TInterface i false)
  | Sub_InterfaceLeft :
      forall i i1 i2,
        interfaceLookup P i = Some (ExtInterface i i1 i2) ->
        subtypeOf P (TInterface i false) (TInterface i1 false)
  | Sub_InterfaceRight :
      forall i i1 i2,
        interfaceLookup P i = Some (ExtInterface i i1 i2) ->
        subtypeOf P (TInterface i false) (TInterface i2 false)
  | Sub_Refl :
      forall t,
        wfType P t ->
        subtypeOf P t t
  ...
  | Sub_Nullable :
      forall t1 t2,
        subtypeOf P t1 t2 ->
        subtypeOf P (nullable t1) (nullable t2)
  | Sub_Nullable_R :
      forall t1 t2,
        subtypeOf P t1 t2 ->
        subtypeOf P t1 (nullable t2)
.
...
Tactic Notation "subtypeOf_cases" tactic(first) ident(c) :=
  ...
  | Case_aux c "Sub_Trans"
  | Case_aux c "Sub_Nullable" | Case_aux c "Sub_Nullable_R"].
...
Inductive hasType (P : program) (Gamma : env) : expr -> ty -> Prop :=
  ...
  | T_New :
      forall c,
        wfEnv P Gamma ->
        classLookup P c <> None ->
        P ; Gamma |- ENew c \in TClass c false
  | T_Call :
      forall x m e t t1 t2 msigs y,
        P ; Gamma |- (EVar x) \in t1 -> (*Gamma (env_var x) = Some t1 ->*)
  ...
  | T_Select :
      forall x f c n t fs,
        P ; Gamma |- (EVar x) \in TClass c n ->
        fields P (TClass c n) = Some fs ->
        fieldLookup fs f = Some (Field f t) ->
        P ; Gamma |- ESelect x f \in t
  | T_Update :
      forall c n x f e t fs,
        P ; Gamma |- (EVar x) \in TClass c n ->
        fields P (TClass c n) = Some fs ->
        fieldLookup fs f = Some (Field f t) ->
        P ; Gamma |- e \in t ->
