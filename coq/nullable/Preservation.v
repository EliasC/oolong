...
Proof with eauto using subtypeOf_isNullable, subtypeOf with env.
  ...
  + Case "T_New".
    ...
    exists (extend Gamma (env_loc (length H)) (TClass c false)).
    assert (wfEnv P (extend Gamma (env_loc (length H)) (TClass c false)))...
  ...
  + Case "T_Call".
    ...
    - SCase "EvalCall". clear IHhasType2.
      inv hasType1.
      assert (Hsub: subtypeOf P (TClass c false) t1).
        wfEnvLookup. rewrite_and_invert.
        inv hasType. apply heapMirrorsEnv with (P := P) (Gamma := Gamma) in H15...
        rewrite_and_invert.
      assert (wfC: wfType P (TClass c false))...
      ...
      exists (extend
                (extend
                   Gamma (env_var (DV (DVar n))) (TClass c false))
            ...
            - eapply hasType_subst; eauto 3 with env.
              ...
              with (Gamma := (extend
                                (extend empty (env_var (SV y)) t2)
                                 (env_var (SV this)) (TClass c false)));
                eauto 4 using wfSubsumption_extend with env.
  ...
  + Case "T_Select".
    ...
    - SCase "EvalSelect". clear IHhasType.
      ...
      assert (exists nll, t2 = TClass c nll); subst...
      destruct H as [nll]. subst.
      assert (c0 = c)
  ...
  + Case "T_Update".
    ...
    - SCase "EvalUpdate". clear IHhasType1. clear IHhasType2.
      ...
      assert (Heq: c1 = c)
        by (apply subtypeOf_class in H7; inv H7; inv_eq); subst.
      rewrite_and_invert.
      exists Gamma. split...
      inverts wfL as ? ? wfL.
      inv wfL...
